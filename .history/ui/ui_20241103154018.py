...
class TaskAppUI:
    ...
    def edit_task(self):
        ...
        if selected_task:
            ...
            task_name = self.task_table.tree.item(selected_task[0])['values'][0]
            priority = self.task_table.tree.item(selected_task[0])['values'][0]
            due_date = self.task_table.tree.item(selected_task[0])['values'][0]
            
            task = self.task_entry.get()
            priority = self.priority_var.get()
            due_date = self.due_date_entry.get()
            if validate_data(due_date):
                self.task_manager.edit_task(task_id, task, priority, due_date)
