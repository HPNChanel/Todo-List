...
from task.task_manager import add_new_task, get_all_tasks, complete_task, db_delete_task, edit_task

def create_ui():
  root = tk.Tk()  #* Tk(): constructor method used to create the main application window
  root.title("Todo List")
  root.geometry("1255x450")
  ...
  add_button = tk.Button(form_frame, text="Add Task", command=add_task_ui, width=20, bg="#4CAF50", fg="white")
  add_button.grid(row=3, column=1, pady=10)
  
  #* Task Display Frame
  task_frame = tk.Frame(root, bg="#f0f0f0")
  task_frame.pack(padx=15, pady=10, fill="both", expand=True)
  
  task_table = TaskTable(task_frame)
