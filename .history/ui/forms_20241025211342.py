...
class TaskForm:
  def __init__(self, root, submit_callback, task_id=None, task=None, priority=None, due_date=None):
    ...
    self.form_window = tk.Toplevel(self.root)  #* Toplevel() is used to create a top-level window, just an independent window that pops up separately from the main application 
    self.form_window.title("Add New Task")  #* This is the title of the window
