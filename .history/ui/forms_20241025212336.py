...
class TaskForm:
  def __init__(self, root, submit_callback, task_id=None, task=None, priority=None, due_date=None):
    ...
    tk.Label(self.form_window, text="Priority (Low, Medium, High):").pack()
    self.priority_entry = tk.Entry(self.form_window)
    self.priority_entry().pack()
    if priority:
      self.priority_entry.insert(0, priority)
      
    
    tk.Label(self.form_window, text="Due Date (YYYY-MM-DD):").pack()
    self.due_date_entry = tk.Entry(self.form_window)
