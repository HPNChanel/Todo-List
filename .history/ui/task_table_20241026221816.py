...
from task.task_manager import get_all_tasks, complete_task, remove_task, update_task
...
class TaskTable(tk.Frame):
  ...
  def edit_task(self):
    ...
    if selected_item:
      ...
      tk.Label(edit_window, text="Due Date (YYYY-MM-DD):").grid(row=2, column=0)
      ...
      def save_changes():
        ...
        if new_task and new_priority and new_due_date:
          ...
          tk.messagebox.showwarning("Input Error", "Please fill all fields.")
      
      save_button = tk.Button(edit_window, text="Save Changes", command=save_changes, bg="#4CAF50", fg="#fff")
      save_button.grid(row=3, column=1, pady=10)
    
    else:
      tk.messagebox.showwarning("Selection Error", "Please select a task to edit.")