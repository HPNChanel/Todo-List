...
class TaskTable(tk.Frame):
  ...
  def edit_task(self):
    selected_item = self.tree.selection()[0]
    task_id, task, priority, due_date, status = self.tree.item(selected_item)['values']
    form = TaskForm(self, self.update_task_in_table, task_id, task, priority, due_date)
    form.show()
  
  def update_task_in_table(self, task_id, task, priority, due_date):
